...
census_2010to2019_raw <- read_csv(
  "Population Estimates and Projections/U.S. Census Bureau_2010 to 2019 State Population Estimates_All Components_Downloaded 10.25.2024.csv",
) %>% as.data.frame()

census_2020to2023_raw <- read_csv(
  "Population Estimates and Projections/U.S. Census Bureau_2020 to 2023 State Population Estimates_All Components_Downloaded 10.25.2024.csv"
) %>% as.data.frame()

census_idb_2010to2023_raw <- read_csv(
  "Population Estimates and Projections/U.S. Census Bureau_2010 to 2023 IDB Population Estimates_Some Components_Downloaded 10.25.2024.csv",
) %>% as.data.frame()
...
census_2020 <- census_2020to2023_depAg_raw %>% .[.$Area_Name %in% search_space, c("Area_Name", "CENSUS_2020_POP")] %>% 
  distinct() %>% `colnames<-`(c("NAME", "CENSUS2020POP"))
...
us_regions_divisions$States %>% .[. %!in% search_space] %>% length() == 0
unique(us_regions_divisions$Region) %>% .[. %!in% search_space] %>% length() == 0
unique(us_regions_divisions$Division) %>% .[. %!in% search_space] %>% length() == 0
...
filtered_search_space <- census_2020[census_2020[, "NAME"] %!in% c("United States", "Puerto Rico"), ] %>% 
  `colnames<-`(c("NAME", "CENSUS2020POP")) %>% `rownames<-`(NULL)
...
unique_names <- cbind(
  ...
  census_2010to2019_raw[-c(1:6)] %>% colnames() %>% 
    ...
    str_replace_all(., "[^[:alnum:]]|[0-9]{1,4}", "") %>% table() %>% 
...
as.vector(unique_names$`2010to2019`) %>% .[. %!in% colnames(census_idb_2010to2023_raw)]
...
extra_counts2010to2019 <- census_2010to2019_raw$NAME %>% .[. %!in% us_regions]
extra_counts2020to2023 <- census_2020to2023_raw$NAME %>% .[. %!in% us_regions]

# See which entries are and are not included in the smaller set.
matched_counts <- extra_counts2020to2023 %>% .[. %in% extra_counts2010to2019]
matched_counts

missing_counts <- extra_counts2020to2023 %>% .[. %!in% extra_counts2010to2019]
missing_counts
...
census_2010to2019_raw <- rbind(
  ...
  aggregate(census_2010_regions[, -c(1:2)], list(census_2010_regions$Division),
    sum) %>% `colnames<-`(c("NAME", colnames(.)[-1]))
)
...
use_names <- unique_names$`2010to2019` %>% as.vector() %>% .[-c(2, 5, 7)]
...
write.csv(census_2010to2023, "Population Estimates and Projections/US_Census Population Estimates_2010 to 2023.csv", row.names = FALSE)
